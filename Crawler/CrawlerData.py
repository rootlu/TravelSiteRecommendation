...
PROCESS_URL_NUM = 40000  # 每30000条url，创建一个进程
...
class CrawlerData:
    ...
    def get_route_info(self, url_list):
        ...
        for url in url_list:
            ...
            route_info_dict['satisfaction'] = int(tag_grade.get_text(strip=True)[:-1]) if tag_grade is not None else 0
            # 行程概要
            ...
            # 插入新数据到数据表 routeinfo
            self.db_helper.insert_into_routeinfo(url_id, route_info_dict['title'], route_info_dict['satisfaction'],
                                                 route_info_dict['summary'], route_info_dict['text'])
            ...
            script_regex = re.compile(r'window\.pageData[\S\s]+?departCityInfo":(?P<route_departure>.*?),"backCityInfo',
                                      re.M)
            script_ret = script_regex.search(detail_page_html)
            if script_ret is None:
                continue
            else:
                route_departure_list = json.loads(script_ret.group('route_departure'))
                if route_departure_list is None:
                    continue
                else:
                    for item in route_departure_list:
                        departure = item['name'] if item['name'] is not None else 'UNKNOWN'
                        price = int(item['price']) if item['price'] is not None else 0
                    self.db_helper.insert_into_routedep(url_id, departure, price)
    ...
    def get_route_comment(self, url_id, url):
        ...
        outline_comment = self.request_helper.send_request(outline_comment_url)
        ...
        for i in range(total_pages):
            ...
            detail_comment = self.request_helper.send_request(detail_comment_url)
            # comment_json = json.load(comment_page)
            self.db_helper.insert_into_routecom(url_id, outline_comment, detail_comment)
...
if __name__ == '__main__':
    ...
    total_url_list = crawler_obj.db_helper.select_all('routeurl')
    crawler_obj.crawl_data(total_url_list)  # 首次运行程序，抓取page_list中的网页